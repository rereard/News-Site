...
const Covid19 = () => {
    ...
    useEffect(() => {
        dispatch(resetNews())
        dispatch(fetchNews(`https://newsapi.org/v2/everything?q=covid&sortBy=publishedAt&searchIn=description,title&pageSize=20&page=${page}&language=en&apiKey=${key}`))
    }, []);
    return(
        <div className="w-11/12 flex justify-center mt-20 flex-col">
            <Header title={"COVID-19's News"}/>
            {errorMessage && (
            ...
            <div className="text-center my-5">
                {news.length < totalResult && !isPending && !errorMessage ? (
